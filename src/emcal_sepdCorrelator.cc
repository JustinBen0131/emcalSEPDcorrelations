...
void emcal_sepdCorrelator::doMbdQA(const std::vector<std::string>& trig)
{
  ...
  m_mbdQ = 0.;//==========================================================================
    ...
    // ================================================================
    //  emcal_sepdCorrelator::End  –  enhanced diagnostics
    // ================================================================
    int emcal_sepdCorrelator::End(PHCompositeNode*)
    {
      COUT_BLUE("Writing output…");

      /* 0. basic TFile sanity check ------------------------------------ */
      if (!out)
      {
        std::cerr << CLR_YELLOW << "[End]  TFile* 'out' is nullptr – "
                  << "nothing to write\n" << CLR_RESET;
        return Fun4AllReturnCodes::ABORTEVENT;
      }
      if (!out->IsOpen())
      {
        std::cerr << CLR_YELLOW << "[End]  Output file is *not* open ("
                  << out->GetName() << ")\n" << CLR_RESET;
        return Fun4AllReturnCodes::ABORTEVENT;
      }

      /* book‑keeping counters ------------------------------------------ */
      std::size_t nHistExpected = 0;   // all objects in the map
      std::size_t nHistWritten  = 0;   // Write() returned >0

      /* 1. loop over trigger sub‑directories --------------------------- */
      for (auto& [trig, hMap] : qaHistogramsByTrigger)
      {
        ...
        if (!dir)
        {
          std::cerr << CLR_YELLOW << "[End]  Cannot create directory '"
                    << trig << "' – skipping this trigger\n" << CLR_RESET;
          continue;
        }
        dir->cd();
        ...
        /* 1a. iterate over objects inside this trigger ------------------ */
        for (auto& [key, obj] : hMap)
        {
          ++nHistExpected;
          ++nExpThis;

          TH1* h = dynamic_cast<TH1*>(obj);  // catches TH1, TH2, TProfile …

          if (!h)
          {
            std::cerr << CLR_YELLOW << "[End]  Object '" << key
                      << "' in trigger '" << trig
                      << "' is not a TH1* – skipped\n" << CLR_RESET;
            continue;
          }

          if (h->GetEntries() == 0)
          {
            std::cerr << CLR_YELLOW << "[End]  Histogram '" << key
                      << "' (trigger " << trig << ") has 0 entries – not written\n"
                      << CLR_RESET;
            continue;
          }

          /* 1b. try to write, but protect against ROOT errors ----------- */
          try
          {
            const auto bytes = h->Write("", TObject::kOverwrite);
            if (bytes > 0)
            {
              ++nHistWritten;
              ++nWrtThis;
            ...
            {
              std::cerr << CLR_YELLOW << "[End]  Write() returned 0 for '"
                        << key << "' (trigger " << trig << ")\n" << CLR_RESET;
            }
          }
          catch (const std::exception& e)
          {
            std::cerr << CLR_YELLOW << "[End]  Exception while writing '"
                      << key << "' – " << e.what() << "\n" << CLR_RESET;
          }
        } // object loop

        /* per‑trigger summary ------------------------------------------ */
        LOG(1, CLR_GREEN, "[End]  trigger '" << trig << "': "
            << nWrtThis << " / " << nExpThis << " histograms written");
        out->cd();      // always return to root dir before next trigger
      } // trigger loop

      /* 2. close the file cleanly --------------------------------------- */
      LOG(1, CLR_GREEN, "[End]  writing TFile footer and closing ("
          << nHistWritten << " / " << nHistExpected << " objects written)");
      ...
      {
        std::cerr << CLR_YELLOW << "[End]  Exception during TFile::Write/Close – "
                  << e.what() << "\n" << CLR_RESET;
        delete out;
        out = nullptr;
        return Fun4AllReturnCodes::ABORTEVENT;
      ...
      /* 3. optional human‑readable summary ------------------------------ */
      if (Verbosity() > 0)
      {
