...
private:
    ...
    bool m_mapsBooked = false;        ///< flag: hit‑maps booked in InitRun()
    
    struct CutStat
    {
        std::size_t tested  = 0;   ///< pairs examined in this cut cell
        std::size_t failE   = 0;   ///< rejected by E min
        std::size_t failChi = 0;   ///< rejected by χ² max
        std::size_t failAsy = 0;   ///< rejected by α max
        std::size_t passed  = 0;   ///< survived all three cuts
    };
